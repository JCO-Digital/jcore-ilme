...
const getSticky = () => {
  document.querySelectorAll('[data-jsticky]').forEach((sticky, i) => {
...
const getScroll = () => {
  document.querySelectorAll('[data-jscroll=\'true\']').forEach((scroll) => {
    ...
    const scrollStart = scroll.dataset.scrollstart ? scroll.dataset.scrollstart : threshold
  ...
  document.querySelectorAll('[data-jtoggle]').forEach((toggle) => {
    ...
    if (('jhover' in toggle.dataset)) {
      ...
      toggle.addEventListener('mouseleave', () => {
        if (toggle.getAttribute('aria-expanded') === 'true') toggleHandler(toggleItem, false)
      })
    } else {
...
const getFocus = () => {
  document.querySelectorAll('[data-jfocus]').forEach((element) => {
...
const toggleHandler = (toggleItem, forcedState = undefined) => {
  const activate = (forcedState === undefined) ? !hasClass(toggleItem.element, toggleItem.targetClass) : forcedState
